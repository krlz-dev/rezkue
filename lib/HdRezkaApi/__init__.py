...
class HdRezkaApi():
	...
	def getStream(self, season=None, episode=None, translation=None, index=0):
		def makeRequest(data):
			print(f"[CUSTOM_LIB] Making request to {self.origin}/ajax/get_cdn_series/")
			print(f"[CUSTOM_LIB] Headers: {self.HEADERS}")
			print(f"[CUSTOM_LIB] Data: {data}")

			r = requests.post(f"{self.origin}/ajax/get_cdn_series/", data=data, headers=self.HEADERS, proxies=self.proxy)

			print(f"[CUSTOM_LIB] Response status: {r.status_code}")
			r = r.json()
			print(f"[CUSTOM_LIB] Response JSON: {r}")

			if r['success']:
				# Validate that 'url' is actually a string, not False/bool
				if not isinstance(r['url'], str):
					print(f"[CUSTOM_LIB] ERROR: API returned success=True but url is {type(r['url'])}: {r['url']}")
					print(f"[CUSTOM_LIB] Full response: {r}")
					raise ValueError(f"API blocking detected: url is {type(r['url']).__name__} instead of string. The server may be blocking requests.")

				if not r['url'] or r['url'] == 'false' or r['url'] == 'False':
					print(f"[CUSTOM_LIB] ERROR: API returned empty or false URL")
					raise ValueError("API returned empty URL. The server may be blocking requests.")

				print(f"[CUSTOM_LIB] Valid URL received, decoding...")
				...
				print(f"[CUSTOM_LIB] Stream successfully created with {len(list(stream.videos.keys()))} qualities")
				return stream
			else:
				print(f"[CUSTOM_LIB] ERROR: API returned success=False")
				raise ValueError(f"API request failed: {r}")
